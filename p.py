...
",
    ...
    # Uses .split() like Super 3s
    valid_words = word_text.upper().split()
    
    
    # Lower chance of J, X, Q, Z
    letters = "AAAAAAAEEEEEEEIIIIIOOOOOUUUUBCDFGHKLMNPRSTVWY"
    ...
    def generate_word():
        ...
        # 45% chance to pick a real word
        if valid_words and random.random() < 0.45:
            return random.choice(valid_words)
    
        # Otherwise make a fake word
        while True:
            word = generate_fake_word()
    
            # Avoid weird letters
            if word.count("Q") + word.count("X") + word.count("Z") + word.count("J") <= 1:
                return word
    ...
    if st.session_state.current_word:
        ...
        if st.button("Submit"):
            ...
            is_real = st.session_state.current_word in valid_words
    
            if (answer == "Yes" and is_real) or (answer == "No" and not is_real):
                st.success("Correct!")
                st.session_state.score += 1
            else:
                st.error("Wrong!")
